angular.module("bonApp").controller('ShowFoodItemsController', function($scope, $window, $http, $location, $timeout, dataSharingService, ModalService) {
	...
	MakeRequest = function(requestUrl){
		var request = $http({
			...
			request.success(function (data) {
				...
				//Populating items list which will be then shown to the user 
				$scope.items = [];
				items = [];
				...
				$scope.items = items;
				if(requestUrl == "http://127.0.0.1:3000/showRemovedFoodItems") {
					$scope.regularItems = false;
				}
				else {
	...
	$scope.deleteOpenQuestionWindow = function(item) {
	...
	$scope.undeleteOpenQuestionWindow = function(item) {
	    ...
	    ModalService.showModal({
	      ...
	      controller: "ModalWindowController",
	      title: "",
	      textToShow: "Are sure you want to recover "+item.name+"?",
	      time: 8000,
	      preClose: (modal) => { modal.element.modal('hide'); }
	    }).then(function(modal) {
	      modal.element.modal();
	      modal.close.then(function(result) {
	      		//If user answered Yes the item is to be deleted
	      		if(result){
	      			$scope.UndeleteItem($scope.itemToDelete);
	      		}
	      });
	    });
	};
	...
	$scope.ReloadAllImages = function(){
		MakeRequest("http://127.0.0.1:3000/reloadAllImages");
	}
